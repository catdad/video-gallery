...
const getMeta = async ({ video }) => {
  ...
  const { thumbnail, metaPath, videoPath, cameraName } = getResources(video);
...
const sync = async ({ date, fullSync = false } = {}) => {
  ...
  syncCache.set(key, Promise.resolve().then(async () => {
    ...
    console.time('glob list');
    const [videos, cacheList] = await Promise.all([
      list(),
      (async () => new Set(await globby('*.jpg', { cwd: cache })))()
    ]);
    console.timeEnd('glob list');
    ...
    for (const video of videos) {
      try {
        ...
        const f = formatDate(new Date(meta.date));

        if (!date || fullSync === true || date === f) {
          result.push(meta);
  ...
  const result = await syncCache.get(key);
